...
# New version (using local account with a smaller UID)
HOST_USER ?= $(strip $(if $(USE_FOR_DOCKER),$(USE_FOR_DOCKER),nodummy))
HOST_UID ?= $(strip $(if $(shell id -u $(HOST_USER)),$(shell id -u $(HOST_USER)),4000))
else
# allow override by adding user= and/ or uid=  (lowercase!).
# uid= defaults to 0 if user= set (i.e. root).
HOST_USER = $(USE_FOR_DOCKER)
